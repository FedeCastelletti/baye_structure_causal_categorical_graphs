mcmc_dag_param = function(Y, S, a, a_pi, b_pi, ne = NULL, verbose = FALSE, vset = NULL){
  ...
  I.cal  = sapply(1:ncol(Y), function(j) length(unique(Y[,j])))
  ...
  return(out = list(Y = Y, Dags = Dags, Theta = Theta, Out_causal = Out_causal))

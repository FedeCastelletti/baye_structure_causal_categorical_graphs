...
marg_j = function(j, a, N, I.cal, I.size, dag){
  ...
  N.j.obs   = aggregate(N$freq, by = as.list(N[fa.j]), FUN = sum)$x
  N.j.zeros = rep(0, prod(I.cal[fa.j]) - length(N.j.obs))
  ...
  if(length(pa.j) == 0){
    ...
    return((lgamma(a) - lgamma(a + sum(N.j))) + 
             sum(lgamma(a + N.j)) - length(N.j)*lgamma(a))
    ...
    return((length(N.pa)*lgamma(a/length(N.pa)) - sum(lgamma(a/length(N.pa) + N.pa))) + 
             sum(lgamma(a/length(N.pa) + N.j)) - length(N.j)*lgamma(a/length(N.pa)))
